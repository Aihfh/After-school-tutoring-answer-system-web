...
<body>
<form action="MessmodifyServlet" method="post">
    <table id="customers">
        <tr>
            <th>MessageId留言编号</th>
            <th>Name留言人</th>
        ...
        <c:forEach var="m" items = "${applicationScope.nowmess}" varStatus="status">
            <tr>

                <td><span><label> <input type="text" name="fu" value="${m.messageId}" readonly></label></span></td>
                <td><span>${m.nname}</span></td>
                <td><label>
                    <input type="text" name="newtitle" value="${m.ttitle}" maxlength="20" required>
                </label></td>
                <td><span><label><input type="text" name="newcontext" value="${m.ccontext}" required></label> </span></td>
                <td><span>${m.ddate} </span></td>
                <td><span>${m.classtype}</span></td>
                <td><button type="submit" class="btn btn-xs btn-success">确认</button></td>
            </tr>
        </c:forEach>
        <!--{applicationScope.math.shuxingm}-->
    </table>
</form>
